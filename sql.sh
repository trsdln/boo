...
source_deploy_conf_for_sql() {
  ...
  # use IAM based auth if no Postgres URL:
  if [ -z "${POSTGRES_URL+x}" ]; then
    local PGUSER=$(gcloud config list account --format "value(core.account)")
    local PGUSER=$(node -p "encodeURIComponent('${PGUSER}').replace('.gserviceaccount.com','')")
